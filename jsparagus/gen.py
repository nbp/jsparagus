...
class ParseTable:
        ...
        def transform():
            for s in self.states:
                ...
                if len(filter_by_replay_term) == 1:
                    ...
                    self.add_edge(filter_state, replay_term, dest)
                else:
                    for replay_term, filter_term in filter_by_replay_term.items():
                        dest = replay_term.replay_steps[-1]
                        ...
                        if is_new:
                            self.add_edge(replay_state, replay_term, dest.index)
                        #print(replay_state)
    ...
    def fold_replay_unwind(self, verbose, progress):
        ...
        def try_transform(s):
            ...
            if len(replayed) == unshifted:
                ...
                locations = OrderedFrozenSet(replay_dest.locations)
                delayed = OrderedFrozenSet(itertools.chain(replay_dest.delayed_actions, [implicit_replay_term]))
                ...
                if is_new:
                    ...
                    self.add_edge(unwind_state, new_unwind_term, unwind_dest)
